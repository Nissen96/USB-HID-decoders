...
def decode(raw_data):
    ...
    # Lines can have a few different formats
...
def draw(clicks, xs, ys, draw_moves=True, mousedown_only=True, draw_clicks=False, animate=False, click_delay=2):
    ...
    for click, x, y in zip(clicks, xs, ys):
        ...
        if animate:
            # Use provided delay on new clicks
            if draw_clicks and click and not mousedown:
                plt.pause(click_delay)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Decode USB Mouse HID data', formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('-d', '--delay', type=int, default=2, metavar='s', help='animation delay in seconds after each click (default: %(default)s)')
...
def main():
    ...
    signal.signal(signal.SIGINT, lambda signum, frame: exit())  # Graceful close on Ctrl+C during drawing
